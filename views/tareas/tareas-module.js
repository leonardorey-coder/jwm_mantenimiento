...
function actualizarResumenTareas() {
  ...
  const pendientes = tareasFiltradasBase.filter(
    (t) => t.estado === 'pendiente'
  ).length;
  const enProceso = tareasFiltradasBase.filter(
    (t) => t.estado === 'en_proceso'
  ).length;
  const completadas = tareasFiltradasBase.filter(
    (t) => t.estado === 'completada'
  ).length;
  const canceladas = tareasFiltradasBase.filter(
...
function configurarFiltroResumenTareas() {
  ...
  listaResumen.addEventListener('click', (event) => {
    ...
    listaResumen.querySelectorAll('[data-resumen-filtro]').forEach((el) => {
      el.classList.toggle(
        'is-selected',
        el.dataset.resumenFiltro === filtroResumenActual
      );
    });
...
async function actualizarPanelServiciosTareas() {
  ...
  const mensajeNoServicios = document.getElementById(
    'mensajeNoServiciosTareas'
  );
  ...
  if (!todosLosServiciosCache || todosLosServiciosCache.length === 0) {
    try {
      ...
      if (response.ok) {
        ...
        console.log(
          `✅ [SERVICIOS-PANEL] ${todosLosServiciosCache.length} servicios cargados`
        );
      }
    } catch (error) {
  ...
  if (hayServiciosEspacios && espaciosComunesCache.length === 0) {
    try {
      ...
      const response = await fetch(
        `${API_URL.replace('/api', '')}/api/espacios-comunes`,
        { headers: obtenerHeadersConAuth() }
      );
      if (response.ok) {
        ...
        console.log(
          `✅ [SERVICIOS-PANEL] ${espaciosComunesCache.length} espacios cargados`
        );
      }
    } catch (error) {
      console.warn(
        '⚠️ [SERVICIOS-PANEL] No se pudieron cargar espacios comunes:',
        error
      );
    }
  }
  ...
  const obtenerUbicacion = (servicio) => {
    ...
    if (servicio.espacio_comun_id) {
      ...
      const espacio = espaciosComunesCache.find(
        (e) => e.id === servicio.espacio_comun_id
      );
  ...
  let serviciosFiltrados = todosServicios.filter((servicio) => {
    ...
    if (rolFiltro === 'para-mi') {
      ...
      const asignadoA =
        servicio.usuario_asignado_id || servicio.usuario_asignado;
      ...
      const rolServicio = servicio.usuario_asignado_rol_nombre;
      return (
        rolServicio && rolServicio.toLowerCase() === rolFiltro.toLowerCase()
      );
    }
  ...
  const serviciosVistos = JSON.parse(
    localStorage.getItem('serviciosVistos') || '[]'
  );
  ...
  serviciosFiltrados.forEach((servicio) => {
    ...
    li.setAttribute(
      'data-es-espacio',
      servicio.espacio_comun_id ? 'true' : 'false'
    );
    ...
    li.addEventListener('click', () => {
      ...
      if (!serviciosVistos.includes(servicio.id)) {
        ...
        localStorage.setItem(
          'serviciosVistos',
          JSON.stringify(serviciosVistos)
        );
