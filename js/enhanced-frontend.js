...
function initializeAnimations() {
  ...
  anime({
    targets:
      '.brutalist-card:not(#listaCuartos .brutalist-card):not(#listaEspacios .brutalist-card)',
    translateY: [50, 0],
    opacity: [0, 1],
...
function setupButtonAnimations() {
  const buttons = document.querySelectorAll(
    '.brutalist-card__button, .container-btn-file, .Documents-btn'
  );
      ...
      setTimeout(() => {
        ...
        if (window.mostrarAlertaBlur)
          window.mostrarAlertaBlur(
            'Bitácora exportada exitosamente',
            'success'
          );
      }, 2000);
    }, 500);
  ...
  if (spinner) {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        spinner.style.display = 'none';
        if (window.mostrarAlertaBlur)
          window.mostrarAlertaBlur('Excel generado exitosamente', 'success');
      }, 2000);
    }, 500);
  }
}
...
function verHistorialFiltros() {
  if (window.mostrarAlertaBlur)
    window.mostrarAlertaBlur('Abriendo historial de filtros...', 'info');
...
function loadInitialData() {
  ...
  console.log(
    'Frontend mejorado cargado - Backend mantiene su lógica original'
  );
}
...
setTimeout(() => {
  if (typeof window.generarMantenimientosHTMLSimple !== 'undefined') {
    console.log(
      '⚠️ Sobrescribiendo generarMantenimientosHTMLSimple con nuevo estilo...'
    );
  }

  window.generarMantenimientosHTMLSimpleOriginal =
    window.generarMantenimientosHTMLSimple;
...
function mostrarFormularioAccion(formId) {
  ...
  const todosLosFormularios = document.querySelectorAll(
...
function inicializarCanvasFirma() {
  ...
  /**
   * Función auxiliar para dibujar en el canvas
