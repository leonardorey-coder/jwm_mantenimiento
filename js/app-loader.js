...
`;
  ...
  document.addEventListener('keydown', function (e) {
    if (e.key === 'Escape') {
      ...
      // Salir del modo edición de la card (habitaciones / espacios comunes)
      // Si el foco está dentro de una card, cerrar esa primero.
      const activeElement = document.activeElement;
      const activeCard = activeElement?.closest?.(
        'li[id^="cuarto-"], li[id^="espacio-"]'
      );

      if (activeCard?.id?.startsWith('cuarto-')) {
        const cuartoId = parseInt(activeCard.id.replace('cuarto-', ''), 10);
        const btnEditar = document.getElementById(`btn-editar-${cuartoId}`);
        if (
          btnEditar?.classList.contains('modo-edicion-activo') &&
          typeof window.toggleModoEdicion === 'function'
        ) {
          window.toggleModoEdicion(cuartoId);
          return;
        }
      }

      if (activeCard?.id?.startsWith('espacio-')) {
        const espacioId = parseInt(activeCard.id.replace('espacio-', ''), 10);
        const btnEditar = document.getElementById(
          `btn-editar-espacio-${espacioId}`
        );
        if (
          btnEditar?.classList.contains('modo-edicion-activo') &&
          typeof window.toggleModoEdicionEspacio === 'function'
        ) {
          window.toggleModoEdicionEspacio(espacioId);
          return;
        }
      }

      // Fallback: cerrar la última card que esté en modo edición.
      const btnEditarEspacioActivo = Array.from(
        document.querySelectorAll(
          '[id^="btn-editar-espacio-"].modo-edicion-activo'
        )
      ).pop();

      if (
        btnEditarEspacioActivo &&
        typeof window.toggleModoEdicionEspacio === 'function'
      ) {
        const espacioId = parseInt(
          btnEditarEspacioActivo.id.replace('btn-editar-espacio-', ''),
          10
        );
        if (!Number.isNaN(espacioId)) {
          window.toggleModoEdicionEspacio(espacioId);
          return;
        }
      }

      const btnEditarCuartoActivo = Array.from(
        document.querySelectorAll(
          '[id^="btn-editar-"]:not([id^="btn-editar-espacio-"]).modo-edicion-activo'
        )
      ).pop();

      if (
        btnEditarCuartoActivo &&
        typeof window.toggleModoEdicion === 'function'
      ) {
        const cuartoId = parseInt(
          btnEditarCuartoActivo.id.replace('btn-editar-', ''),
          10
        );
        if (!Number.isNaN(cuartoId)) {
          window.toggleModoEdicion(cuartoId);
        }
      }
    }
