...
class StorageHelper {
  ...
  async saveAuthTokens(tokens, persistent = true) {
    ...
    await Promise.all(
      [
        ...
        sesionId &&
          this.dbManager.setAuth('sesionId', sesionId, 'token', persistent),
      ].filter(Boolean)
    );
  ...
  async clearAuth(persistent = true) {
    ...
    await Promise.all(authKeys.map((key) => this.dbManager.deleteAuth(key)));
  ...
  async saveMantenimientos(mantenimientos) {
    ...
    if (!Array.isArray(mantenimientos)) {
      console.error(
        '❌ [StorageHelper] saveMantenimientos: se esperaba un array'
      );
      return;
    }
    ...
    console.log(
      `✅ [Data] ${result.success} mantenimientos guardados en IndexedDB`
    );
  ...
  async saveAllData(data) {
    ...
    console.log(
      `✅ [Data] ${successful}/${results.length} operaciones completadas`
    );
  ...
  async processSyncQueue(apiBaseUrl) {
    ...
    console.log(
      `🔄 [Sync] Procesando ${operations.length} operaciones pendientes...`
    );
    ...
    for (const operation of operations) {
      try {
        ...
        if (response.ok) {
          ...
          failed++;
          console.error(
            `❌ [Sync] Error en operación ${operation.type}:`,
            response.status
          );
        }
      } catch (error) {
    ...
    console.log(
      `✅ [Sync] Sincronización completada: ${success} exitosas, ${failed} fallidas`
    );
  ...
  async cleanExpiredData() {
    ...
    console.log(
      `🧹 [Maintenance] Limpieza completada: ${cacheDeleted} cache, ${syncDeleted} sync`
    );
  ...
  async clearAllData() {
    ...
    const confirmed = window.electronSafeConfirm
