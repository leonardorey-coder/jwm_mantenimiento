...
function initLoginPage() {
  ...
  if (!window.location.pathname.includes('login.html')) {
    console.log(
      '🔵 [LOGIN-JWT] No estamos en login.html, saltando inicialización'
    );
    return;
  }
  ...
  if (window.electronAPI && window.electronAPI.onBeforeQuit) {
    window.electronAPI.onBeforeQuit(async () => {
      console.log(
        '🛑 [LOGIN-JWT] Recibido evento app:before-quit, cerrando IndexedDB...'
      );
...
async function initializeAuth() {
  console.log(
    '🔵 [LOGIN-JWT] initializeAuth() - Iniciando verificación de autenticación'
  );
  ...
  if (window.electronAPI && window.electronAPI.auth) {
    try {
      console.log(
        '🔵 [LOGIN-JWT] Intentando recuperar sesión de Electron Storage...'
      );
      ...
      if (electronAuth && electronAuth.accessToken) {
        ...
        if (electronAuth.currentUser) {
          localStorage.setItem(
            'currentUser',
            JSON.stringify(electronAuth.currentUser)
          );
        }
      }
    } catch (err) {
...
loginForm.addEventListener('submit', async (e) => {
  ...
  try {
    console.log(
      '🟢 [LOGIN-JWT] Enviando petición de login a:',
    ...
    if (data.success) {
      ...
      console.log('🟢 [LOGIN-JWT] Usuario guardado:', {
        id: userData.id,
...
async function handleForgotPasswordSubmit(event) {
  ...
  if (!nombre || !email) {
    setForgotPasswordFeedback(
      'Nombre y correo son requeridos para registrar la solicitud.',
      'error'
    );
    return;
  }
  ...
  try {
    ...
    setForgotPasswordFeedback(
      data.info ||
        'Solicitud enviada. El administrador te contactará en breve.',
      'success'
    );
    setTimeout(() => {
      closeForgotPasswordModal();
      showMessage(
        'Solicitud enviada. Revisa tu correo o contacta al administrador.',
        'success'
      );
    }, 1200);
  } catch (error) {
    ...
    setForgotPasswordFeedback(
      error.message || 'Ocurrió un error al enviar tu solicitud.',
      'error'
    );
  } finally {
    if (submitBtn) {
...
function showForcePasswordModal(usuario) {
  ...
  setForcePasswordFeedback(
    'Ingresa una contraseña nueva para continuar.',
    'info'
  );
  modal.classList.add('show');
  lockBodyScrollAuth();
...
async function handleForcePasswordSubmit(event) {
  ...
  try {
    const response = await fetchWithAuth(
      `${API_BASE_URL}/api/auth/cambiar-password-obligatorio`,
      {
        method: 'POST',
        body: JSON.stringify({
          nuevoPassword,
    ...
    if (!response.ok) {
      throw new Error(
        data.mensaje || data.error || 'No se pudo actualizar la contraseña'
      );
    }
