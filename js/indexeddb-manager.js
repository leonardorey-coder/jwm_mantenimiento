...
class IndexedDBManager {
  ...
  /**
   * Ejecuta una operación en un object store de IndexedDB
   * @private
   * @param {string} storeName - Nombre del object store
   * @param {string} mode - Modo de transacción ('readonly' o 'readwrite')
   * @param {Function} operation - Función que ejecuta la operación en el store
   * @returns {Promise} Promesa que resuelve con el resultado de la operación
   */
  async _executeStoreOperation(storeName, mode, operation) {
  ...
  async set(storeName, data) {
    return this._executeStoreOperation(storeName, 'readwrite', (store) =>
      store.put(data)
    );
  }
  ...
  async get(storeName, key) {
    return this._executeStoreOperation(storeName, 'readonly', (store) =>
      store.get(key)
    );
  }
  ...
  async getAll(storeName) {
    const result = await this._executeStoreOperation(
      storeName,
      'readonly',
      (store) => store.getAll()
    );
  ...
  async delete(storeName, key) {
    await this._executeStoreOperation(storeName, 'readwrite', (store) =>
      store.delete(key)
    );
  }
