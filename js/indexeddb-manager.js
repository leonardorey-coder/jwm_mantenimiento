    ...
    async init() {
        ...
        this.initPromise = new Promise((resolve, reject) => {
            ...
            request.onupgradeneeded = (event) => {
                ...
                console.log('🔧 [IndexedDB] Actualizando estructura de la base de datos...');
                ...
                if (!db.objectStoreNames.contains('edificios')) {
                    const edificiosStore = db.createObjectStore('edificios', { keyPath: 'id' });
                ...
                if (!db.objectStoreNames.contains('cuartos')) {
                    const cuartosStore = db.createObjectStore('cuartos', { keyPath: 'id' });
                    cuartosStore.createIndex('numero', 'numero', { unique: false });
                    cuartosStore.createIndex('edificio_id', 'edificio_id', { unique: false });
                ...
                if (!db.objectStoreNames.contains('mantenimientos')) {
                    const mantStore = db.createObjectStore('mantenimientos', { keyPath: 'id' });
                    ...
                    mantStore.createIndex('fecha_registro', 'fecha_registro', { unique: false });
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = (event) => {
                ...
                if (cursor) {
                    ...
                    console.log(`🧹 [IndexedDB] Cache limpiado: ${deletedCount} entradas eliminadas`);
        ...
        return new Promise((resolve, reject) => {
            ...
            request.onsuccess = (event) => {
                ...
                if (cursor) {
                    ...
                    if (record.status === 'completed' && record.completedAt < cutoffTime) {
                        cursor.delete();
                        deletedCount++;
                    }
                    cursor.continue();
                } else {
                    console.log(`🧹 [IndexedDB] Cola de sincronización limpiada: ${deletedCount} operaciones eliminadas`);
                    resolve(deletedCount);
                }
            };
        ...
        try {
            ...
            for (const key of cacheKeys) {
                ...
                if (value) {
                    ...
                    } else if (key === 'ultimosMantenimientos' && Array.isArray(parsedValue)) {
