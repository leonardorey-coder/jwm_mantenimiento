...
class StorageWrapper {
    ...
    async init() {
        ...
        this.initPromise = (async () => {
            ...
            console.log(`✅ [StorageWrapper] ${this.storageType} inicializado con ${this.cache.size} elementos`);
        })();
...
class StorageManager {
    ...
    async migrateFromNativeStorage() {
        console.log('🔄 [StorageManager] Verificando migración desde storage nativo...');

        try {
            // Verificar si ya se migró
            const migrationFlag = window.localStorage.getItem('__indexeddb_migrated__');
            if (migrationFlag === 'true') {
                console.log('✅ [StorageManager] Ya se realizó la migración anteriormente');
                return;
            }
...
export function replaceNativeStorage() {
    console.warn('⚠️ [StorageWrapper] Reemplazando localStorage y sessionStorage nativos');
