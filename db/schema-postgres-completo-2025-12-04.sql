...
--
-- Datos iniciales cargados (2025-12-04)
-- Este bloque deja la base lista para uso inmediato con roles, admin, edificios,
-- cuartos completos, espacios comunes y estados de configuración.
--

BEGIN;

-- Estados base para cuartos y espacios comunes
INSERT INTO configuracion_estados (valor, label, descripcion, color, color_hex, color_rgb, color_secundario, icono, prioridad, disponible_para_reserva)
VALUES
    ('disponible', 'Disponible', 'Listo para uso', '#4CAF50', '4CAF50', 'rgb(76, 175, 80)', '#E8F5E9', 'green', 1, true),
    ('ocupado', 'Ocupado', 'Huésped presente', '#2196F3', '2196F3', 'rgb(33, 150, 243)', '#E3F2FD', 'blue', 2, false),
    ('mantenimiento', 'Mantenimiento', 'En servicio técnico', '#FF9800', 'FF9800', 'rgb(255, 152, 0)', '#FFF3E0', 'orange', 3, false),
    ('fuera_servicio', 'Fuera de Servicio', 'Inhabilitado temporalmente', '#616161', '616161', 'rgb(97, 97, 97)', '#F5F5F5', 'gray', 4, false)
ON CONFLICT (valor) DO NOTHING;

SELECT setval('configuracion_estados_id_seq', COALESCE((SELECT MAX(id) FROM configuracion_estados), 1));

-- Roles base
INSERT INTO roles (id, nombre, descripcion, permisos) VALUES
    (1, 'ADMIN', 'Administrador del sistema con acceso total', '{"all":true,"usuarios":true,"exportar_excel":true,"habitaciones":true,"espacios":true,"tareas":true,"checklist":true,"sabanas":true,"alertas":true}'),
    (2, 'SUPERVISOR', 'Supervisor con acceso a reportes y exportación', '{"all":false,"usuarios":false,"exportar_excel":true,"habitaciones":true,"espacios":true,"tareas":true,"checklist":true,"sabanas":true,"alertas":true}'),
    (3, 'TECNICO', 'Técnico de mantenimiento sin gestión de usuarios', '{"all":false,"usuarios":false,"exportar_excel":false,"habitaciones":true,"espacios":true,"tareas":true,"checklist":true,"sabanas":false,"alertas":true}')
ON CONFLICT (nombre) DO NOTHING;

SELECT setval('roles_id_seq', COALESCE((SELECT MAX(id) FROM roles), 1));

-- Usuario admin (Fidel Cruz Lozada)
INSERT INTO usuarios (id, nombre, email, password_hash, rol_id, activo, telefono, departamento, numero_empleado, requiere_cambio_password, intentos_fallidos, bloqueado_hasta, notas_admin)
VALUES (
    1,
    'Fidel Cruz Lozada',
    'fcruz@grupodiestra.com',
    crypt('fcl1020', gen_salt('bf')),
    1,
    true,
    '+52 624 237 1063',
    'Mantenimiento',
...
SELECT setval('edificios_id_seq', COALESCE((SELECT MAX(id) FROM edificios), 1));

-- Función auxiliar para asignar estados iniciales
...
SELECT setval('cuartos_id_seq', COALESCE((SELECT MAX(id) FROM cuartos), 1));

-- Espacios comunes base
INSERT INTO espacios_comunes (id, nombre, edificio_id, tipo, descripcion, estado, capacidad, horario_apertura, horario_cierre, activo) VALUES
(1, 'Lobby Principal', 1, 'Lobby', 'Lobby y recepción', 'disponible', 200, '00:00', '23:59', true),
(2, 'Gimnasio', 1, 'Gimnasio', 'Gimnasio equipado', 'disponible', 50, '06:00', '22:00', true),
(3, 'Piscina', 1, 'Piscina', 'Piscina principal', 'disponible', 120, '08:00', '20:00', true),
(4, 'Restaurante', 1, 'Restaurante', 'Restaurante principal del hotel', 'disponible', 100, '07:00', '23:00', true),
(5, 'Spa', 1, 'Spa', 'Spa y masajes', 'disponible', 40, '09:00', '21:00', true)
ON CONFLICT (nombre, edificio_id) DO NOTHING;

SELECT setval('espacios_comunes_id_seq', COALESCE((SELECT MAX(id) FROM espacios_comunes), 1));
